from fastapi import APIRouter, Request, Security, Query, UploadFile, File
...
from core.Response import fail, success, total_result
from core.Utils import encry_password, check_password, random_str
from models.base import User, Role, Access, AccessLog
from schemas.user import CreateUser, UpdateUserInfo, ModifyMobile
...
async def user_del(req: Request, user_id: int):
    ...
    if req.state.user_id == user_id:
        return fail(msg=f"用户ID{user_id}删除失败!")
...
@router.put("/update", summary="用户修改",
...
async def user_list(
    ...
    return total_result(data, total, 0, page=current, pagesize=pageSize, msg="success")
