...
@router.post("/create", summary="角色添加", dependencies=[Security(check_permissions, scopes=["role_add"])])
...
@router.delete("/delete", summary="角色删除", dependencies=[Security(check_permissions, scopes=["role_delete"])])
...
@router.put("/update", summary="角色修改", dependencies=[Security(check_permissions, scopes=["role_update"])])
...
@router.get('/list', summary="角色列表", response_model=RoleList,
...
async def get_all_role(
    ...
    return total_result(code=0, data=data, total=total, page=current, pagesize=pageSize, msg="success")
